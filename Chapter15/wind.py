...
def validate_net(net: model.ModelA2C, env: gym.Env, count: int = 10,
...
if __name__ == "__main__":
    ...
    env_factories = [
        lambda: gym.make("LunarLander-v2", continuous=True, enable_wind=True, gravity=-5.0)
    ...
    test_env = gym.make("LunarLander-v2", continuous=True, render_mode="rgb_array", enable_wind=True, gravity=-5.0)
    test_env = gym.wrappers.TransformReward(test_env, lambda r: r / 100.0)  # Auch Normalisierung des Rewards normalisieren
    test_env = RecordVideo(env=test_env, video_folder="./video_wind", episode_trigger=lambda x: x % 1000 == 0)
    net = model.ModelA2C(env.observation_space.shape[1], env.single_action_space.shape[0]).to(device)
        ...
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            for step_idx, exp in enumerate(exp_source):
                ...
                if step_idx % TEST_ITERS == 0:
                    ts = time.time()
                    rewards, steps = validate_net(net, test_env, device=device)
